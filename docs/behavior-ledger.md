...
| Pain Split in the bounded singles engine | Pinned FireRed `BattleScript_EffectPainSplit`, `Cmd_painsplitdmgcalc`, and own-max HP update; exact effect-91 ROM record | `tests/battle_engine_test.lua`, `tests/battle_scene_controller_test.lua`, `tests/phase4_pain_split_effect_rom_test.lua`; independent review at `a739ddc`, guarded run `35493728001` | Ordered two-sided HP and scene messages accepted within task exclusions; no generic healing, excluded interactions, or visual parity claim |
| Saving and loading | Retail-derived sector/checksum structure in a project schema-2 wrapper; [explicit compatibility/refusal contract](save-format.md) | `tests/save_file_codec_test.lua`: independent legacy/current/truncation, PC-sector fallback and source-locked u32 rollover fixtures; `tests/save_load_roundtrip_test.lua`: full prior-slot preservation and load/resave through wrap | Live save/load; version 1 refused without migration. Partial field model; suffix preservation remains limited; codec fallback does not establish filesystem atomicity |
