...
| Saving and loading | Retail-derived sector/checksum structure in a project schema-2 wrapper; [explicit compatibility/refusal contract](save-format.md) | `tests/save_file_codec_test.lua`: independent legacy/current/truncation and PC-sector fallback fixtures; `tests/save_load_roundtrip_test.lua`: full prior-slot preservation and loaded state | Live save/load; version 1 refused without migration. Partial field model; rollover/suffix limits remain; codec fallback does not establish filesystem atomicity |
