...
| Phase 4B-pre — bridge route for foe-only multi-mon trainers | `ACTIVE` | **[`../tasks/local-worker-bridge-phase4-foe-only-multimon-route.md`](../tasks/local-worker-bridge-phase4-foe-only-multimon-route.md)** | explicit route + probe + independent PASS | Phase 4B eligibility |
| Phase 4B — foe-only multi-mon trainer orchestration | `BLOCKED ON BRIDGE ROUTE` | [`../tasks/phase4-foe-only-multimon-trainer-orchestration.md`](../tasks/phase4-foe-only-multimon-trainer-orchestration.md) | ordered replacements and per-foe reward proof | player replacement and broader trainer matrix |
