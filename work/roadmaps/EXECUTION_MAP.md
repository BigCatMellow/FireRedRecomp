...
Phase 3 playable vertical slice [DONE]
        ↓
Phase 2 renderer/runtime [IN PROGRESS]  ← EXTERNAL REFERENCE BLOCKER
        │
...
| --- | --- | --- | --- | --- |
| Phase 0 — charter/reproducibility | `IN PROGRESS` | [Save counter correction](../tasks/phase0-save-counter-rollover.md) and disjoint [public repository checks](../tasks/phase0-ci-repository-checks.md) | Save contract `63592c3` and first CI receipts independently PASS; exact counter/CI implementation reviews and remaining clean-clone evidence | stronger release baseline |
...
| Phase 4W — Flail/Reversal effect discovery | `REVIEWED PASS` | [`../tasks/phase4-flail-effect-discovery.md`](../tasks/phase4-flail-effect-discovery.md) | effect-99 dynamic-base-power source lock independently passed | separate fail-closed route plan |
| Phase 4X-pre — bridge route for Flail/Reversal | `REVIEWED PASS / PROBE PASS` | [`../tasks/local-worker-bridge-phase4-flail-route.md`](../tasks/local-worker-bridge-phase4-flail-route.md) | config PASS `d0e151e2`; probe `883536b0`, guarded run `34994516318` | Phase 4X implementation |
| Phase 4X — Flail/Reversal effects | `REVIEWED PASS` | [`../tasks/phase4-flail-effects.md`](../tasks/phase4-flail-effects.md) | PASS `656c1f1b`; guarded run `34995558611` and independent canonical review | select next source-locked effect family separately |
| Phase 4Y — Eruption/Water Spout effect discovery | `REVIEWED PASS` | [`../tasks/phase4-eruption-effect-discovery.md`](../tasks/phase4-eruption-effect-discovery.md) | effect-190 current-HP dynamic-power source lock independently passed | separate fail-closed route plan |
| Phase 4Z-pre — bridge route for Eruption/Water Spout | `REVIEWED PASS / PROBE PASS` | [`../tasks/local-worker-bridge-phase4-eruption-route.md`](../tasks/local-worker-bridge-phase4-eruption-route.md) | config PASS `09b25096`; probe `104e7eaf`, guarded run `34996725283` | Phase 4Z implementation |
| Phase 4Z — Eruption/Water Spout effects | `REVIEWED PASS` | [`../tasks/phase4-eruption-effects.md`](../tasks/phase4-eruption-effects.md) | Worker PASS `89cf1dae`; guarded run `34997543211`; canonical range independently reviewed; 213 no-ROM / 220 verified-ROM checks plus focused replay | select next source-locked effect family separately |
| Phase 4AA — Psywave effect discovery | `REVIEWED PASS` | [`../tasks/phase4-psywave-effect-discovery.md`](../tasks/phase4-psywave-effect-discovery.md) | exact effect-88 record, rejection-sampling formula/RNG order, and dependency classification independently passed | separate fail-closed route only |
| Phase 4AB-pre — bridge route for Psywave | `REVIEWED PASS / PROBE PASS` | [`../tasks/local-worker-bridge-phase4-psywave-route.md`](../tasks/local-worker-bridge-phase4-psywave-route.md) | configuration review PASS; probe `2e69f25c`, guarded run `35026144931` | Phase 4AB implementation |
| Phase 4AB — Psywave effect | `REVIEWED PASS` | [`../tasks/phase4-psywave-effects.md`](../tasks/phase4-psywave-effects.md) | Worker PASS `40cfdffe`; guarded run `35026622346`; canonical range independently reviewed; 219 no-ROM / 226 verified-ROM checks plus focused replay | select next source-locked effect family separately |
| Phase 4AC — OHKO effect discovery | `REVIEWED PASS` | [`../tasks/phase4-ohko-effect-discovery.md`](../tasks/phase4-ohko-effect-discovery.md) | four exact records, special level/RNG KO contract, and explicit unrepresented-state exclusions independently passed | separate fail-closed route only |
| Phase 4AD-pre — bridge route for OHKO | `REVIEWED PASS / PROBE PASS` | [`../tasks/local-worker-bridge-phase4-ohko-route.md`](../tasks/local-worker-bridge-phase4-ohko-route.md) | configuration review PASS; probe `31abae7c`, guarded run `35027693336` | Phase 4AD implementation |
| Phase 4AD — OHKO effects | `REVIEWED PASS` | [`../tasks/phase4-ohko-effects.md`](../tasks/phase4-ohko-effects.md) | Worker PASS `3afcd988`; guarded run `35080163638`; canonical range independently reviewed; 225 no-ROM / 232 verified-ROM checks plus full no-ROM suite and focused replay | select next source-locked effect family separately |
| Phase 4AE — Endeavor effect discovery | `REVIEWED PASS` | [`../tasks/phase4-endeavor-effect-discovery.md`](../tasks/phase4-endeavor-effect-discovery.md) | exact effect-189 record, viability-before-accuracy formula/order, explicit Protect/Substitute/Focus Band/Endure exclusions, and dependency classification independently passed | separate fail-closed route plan only |
| Phase 4AF-pre — bridge route for Endeavor | `REVIEWED PASS / PROBE PASS` | [`../tasks/local-worker-bridge-phase4-endeavor-route.md`](../tasks/local-worker-bridge-phase4-endeavor-route.md) | generic move-failure event requires minimal scene/controller-test expansion; amended probe `14c164e9`, guarded run `35206557769` | corrected implementation-scope review |
| Phase 4AF — Endeavor effect | `REVIEWED PASS` | [`../tasks/phase4-endeavor-effects.md`](../tasks/phase4-endeavor-effects.md) | Worker PASS `3a489b94`; guarded run `35207142939`; canonical range independently reviewed with generic failure event/no-faint coverage | select next source-locked effect family separately |
...
| Phase 4AJ-pre — inventory evidence route | `BLOCKED — RUNNER OFFLINE` | [`../tasks/local-worker-bridge-phase4-move-effect-inventory-route.md`](../tasks/local-worker-bridge-phase4-move-effect-inventory-route.md) | Exact configuration `24f39c4` independently PASS; separate probe not submitted because runner is offline | resume same probe when online, then inventory request |
| Phase 4AJ — complete move-effect inventory | `BLOCKED — SOURCE/TEST PREFLIGHT PASS` | [`../tasks/phase4-move-effect-inventory.md`](../tasks/phase4-move-effect-inventory.md) | Preparation `688433a` independently reviewed; exhaustive source partition and 150-file local suite PASS; required private-ROM aggregate evidence absent | restore runner, probe, guarded test/report request, final review before next family |
