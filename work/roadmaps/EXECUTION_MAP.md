...
| Phase 4F — move admission-matrix discovery | `PENDING INDEPENDENT REVIEW` | **[`../tasks/phase4-move-admission-matrix-discovery.md`](../tasks/phase4-move-admission-matrix-discovery.md)** | 216 positive-power ROM records inventoried; 75 represented, one explicit reject, 140 broad-fallback records classified | explicit route for only Phase 4G always-hit effects if review passes |
| Phase 4G-pre — bridge route for always-hit effects | `NOT STARTED` | future bounded task | exact fail-closed surface and probe for effect 17 only | Phase 4G eligibility |
| Phase 4G — always-hit effects | `NOT STARTED` | future bounded task | source-locked no-accuracy-RNG behavior for all six effect-17 ROM records, focused/full/replay/review evidence | next admission-matrix leaf |
