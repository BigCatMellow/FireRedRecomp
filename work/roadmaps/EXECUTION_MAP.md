...
| Phase 4S-pre — bridge route for two-sided replacement state | `READY FOR INDEPENDENT REVIEW` | **[`../tasks/local-worker-bridge-phase4-two-sided-replacement-state-api-route.md`](../tasks/local-worker-bridge-phase4-two-sided-replacement-state-api-route.md)** | explicit allowlist, focused/full/replay commands, staging list, and separate probe | Phase 4S route configuration after PASS |
