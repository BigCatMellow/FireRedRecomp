...
| Phase 3 — playable vertical slice | IN PROGRESS | New game, movement, wild battle, capture, save/load tests; the verified-ROM `scripts/runtime_natural_capture_replay.sh` now proves bounded persistent Mart `0 -> 1` -> Parcel -> Lab `5` -> Dex -> Mart `2` -> real shop -> capture -> K-save -> fresh-process L-load | Oak/title entry and visible retail Mart/Parcel/Dex cutscene parity; see `work/tasks/viridian-parcel-dex-progression.md` |
