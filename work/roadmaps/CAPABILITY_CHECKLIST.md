...
| Phase 3 — playable vertical slice | IN PROGRESS | New game, movement, wild battle, capture, save/load tests; ROM-gated `tests/phase3_exit_path_rom_test.lua` composition replay; runtime smoke proves real post-Oak identity input plus bounded starter→Route 1→wild-loss/whiteout and repeatable normal-menu Route 1 victory; `scripts/runtime_save_restart_replay.sh` proves isolated two-process K-save/L-load persistence; `work/tasks/mart-counter-interaction.md` records the source-backed counter rule | Oak/title entry and a live natural-capture route |
