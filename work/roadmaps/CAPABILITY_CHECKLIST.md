...
| Phase 3 — playable vertical slice | IN PROGRESS | New game, movement, wild battle, capture, save/load tests; ROM-gated `tests/phase3_exit_path_rom_test.lua` composition replay; `scripts/runtime_replay_smoke.sh` proves real post-Oak identity input plus bounded starter→Route 1→wild-loss/whiteout paths | Oak/title entry, player-win/catch route, and two-process disk save/load replay |
