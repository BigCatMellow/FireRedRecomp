...
| `P0-02-CONTRACT` | `READY_FOR_WORKER` | `WORKER` | [Save contract/fixture task](../tasks/phase0-save-version-contract.md) | Seven literal documentation/test paths; no runtime, version/refusal policy, or phase changes. | Focused/full no-ROM evidence, independent review, then next bounded save defect or CI gate. |
| `P0-03-DISCOVERY` | `READY_FOR_REVIEWER` | `REVIEWER` | [CI proof discovery](../tasks/phase0-ci-proof-discovery.md), exact `b5b317b` | Earliest/current public receipts, 273 Lua parses and 128 characterized local destinations; no implementation changes. | Independently verify findings and the one proposed syntax/docs-check successor. |
