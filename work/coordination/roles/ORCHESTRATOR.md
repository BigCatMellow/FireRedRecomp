...
2. Read root `AGENTS.md`, coordination flow, canonical capability checklist, `work/roadmaps/EXECUTION_MAP.md`, current `STATE.json`/`HANDOFF.md`/`TASK_REGISTER.md`, relevant active task, and latest review/evidence for that task.
...
9. A newly created task must use `TASK_CONTRACT_TEMPLATE.md` and state goal, source of truth, prerequisites, MAY CHANGE / MUST NOT CHANGE, acceptance, deterministic evidence, stop/escalation conditions, risk, and completion/handoff.
...
12. Update `TASK_REGISTER.md`, then `STATE.json`, then `HANDOFF.md` last.
