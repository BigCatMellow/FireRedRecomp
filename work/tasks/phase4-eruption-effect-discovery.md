...
`Cmd_scaledamagebyhealthratio` sets transient dynamic power to integer
`floor(attacker HP * stored power / attacker maxHP)`, changing zero to one
(`src/battle_script_commands.c:8986-8997`), then normal Hit performs its
...
other 150-power moves. It needs a separate route and review before code.
